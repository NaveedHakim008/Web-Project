  ...
  if ( !is_mobile_device() && (window.innerWidth > 768)) {
    if ($(window).scrollTop() >= (orgElementTop)) {
      ...
      if ( $(window).scrollTop() >= ( orgElementTop + $('.sticky').innerHeight() ) ) {
        ...
        //41,73,179
        //91,92,96
        $('.sticky').css('position','fixed').css('background-color','rgba(77, 57, 57, 0.99)');
      }
      else {
        $('.sticky').css('position','fixed').css('background-color','rgba(77,57,57, 1)');
      }
      ...
      $('.sticky').css('position','relative').css('background-color','rgba(77,57,57,1)');
    }}
  }
window.is_set1 = true;
